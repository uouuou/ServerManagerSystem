...
func IntSqlStart() {
	...
	if db.Find(&menu).RowsAffected <= 0 {
		...
		db.Exec(menuAdd + " (3, 'SM图床', 0, '/img', 'el-icon-picture-outline', 3, 2, 1, '系统');")
		db.Exec(menuAdd + " (3, '图床管理', 3, '/img/manage', 'el-icon-camera', 1, 2, 1, '系统');")
		db.Exec(menuAdd + " (3, '图片分享', 5, '/img/share', 'el-icon-share', 2, 1, 2, '系统');")
		...
		db.Exec(menuAdd + " (5, '访问控制', 5, '/net/access', 'el-icon-coordinate', 1, 2, 1, '系统');")
		...
		db.Exec(menuAdd + " (7, '行为管理', 0, '/action', 'el-icon-ice-cream-square', 7, 1, 1, '系统');")
		db.Exec(menuAdd + " (7, '接入管理', 7, '/action/register', 'el-icon-guide', 1, 1, 1, '系统');")
		db.Exec(menuAdd + " (7, 'SQL资产', 7, '/action/sql', 'el-icon-coin', 2, 1, 1, '系统');")
		db.Exec(menuAdd + " (7, '更新管理', 7, '/action/update', 'el-icon-sell', 3, 1, 2, '系统');")
		db.Exec(menuAdd + " (7, '组件编译', 7, '/action/build', 'el-icon-ice-cream-round', 4, 2, 1, '系统');")
		db.Exec(menuAdd + " (8, '系统设置', 0, '/setting', 'el-icon-setting', 8, 1, 2, '系统');")
		db.Exec(menuAdd + " (8, '角色管理', 8, '/setting/role', 'el-icon-office-building', 1, 1, 2, '系统');")
		db.Exec(menuAdd + " (8, '用户设置', 8, '/setting/users', 'el-icon-user', 2, 1, 2, '系统');")
		db.Exec(menuAdd + " (8, '文件管理', 8, '/setting/file', 'el-icon-files', 3, 1, 2, '系统');")
